...
- (void) viewDidLoad
{
    ...
    self.session = [[AVCaptureSession alloc] init];// 创建采集会话
    ...
    self.videoDeviceDiscoverySession = [AVCaptureDeviceDiscoverySession discoverySessionWithDeviceTypes:deviceTypes mediaType:AVMediaTypeVideo position:AVCaptureDevicePositionUnspecified];// 多种会话session
    ...
    */
    switch ([AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo])
    {
        ...
        case AVAuthorizationStatusNotDetermined:
        {
            ...
            */
            dispatch_suspend(self.sessionQueue);// 队列挂起
            [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
                if (!granted) {
                    self.setupResult = AVCamSetupResultCameraNotAuthorized;
                }
                dispatch_resume(self.sessionQueue);// 队列运行
            }];
            break;
        }
        default:
    ...
    */
    dispatch_async(self.sessionQueue, ^{// 异步执行
...
{
    if (self.setupResult != AVCamSetupResultSuccess) {// 没成功
        return;
    }
    ...
    */
    ...
    AVCaptureDevice* videoDevice = [AVCaptureDevice defaultDeviceWithDeviceType:AVCaptureDeviceTypeBuiltInDualCamera mediaType:AVMediaTypeVideo position:AVCaptureDevicePositionBack];// 设备初始化
    if (!videoDevice) {
        // If a rear dual camera is not available, default to the rear wide angle camera.
        videoDevice = [AVCaptureDevice defaultDeviceWithDeviceType:AVCaptureDeviceTypeBuiltInWideAngleCamera mediaType:AVMediaTypeVideo position:AVCaptureDevicePositionBack];// 后设备扫描
...
{
    if (captureModeControl.selectedSegmentIndex == AVCamCaptureModePhoto) {// 图片
        self.recordButton.enabled = NO;
...
//            }
    ...
    else if (captureModeControl.selectedSegmentIndex == AVCamCaptureModeMovie) {// 视频
...
- (IBAction) capturePhoto:(id)sender
{
    ...
    */
    AVCaptureVideoOrientation videoPreviewLayerVideoOrientation = self.previewView.videoPreviewLayer.connection.videoOrientation;// 设备方向
